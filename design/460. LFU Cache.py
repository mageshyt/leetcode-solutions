...
class LFUCache:
    ...
    def __init__(self, capacity: int):
        #  dictionary which maps key to count
        #  dictionary which maps count to a orderedDict
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if key in self.cache_key:
            ...
            old_count = self.cache_key[key][1]
            self.cache_key[key][1] += 1
            new_count = old_count + 1
            del self.cache_orderedCount[old_count][key]
            self._updateCacheCountDict(key, value, new_count)
            # update the min count
            if self.min_cnt == old_count and len(self.cache_orderedCount[old_count]) == 0:
                self.min_cnt = new_count
        else:
            ...
            if self.used < self.capacity:
                ...
                # the cache is full
                rm_key, rm_val = self.cache_orderedCount[self.min_cnt].popitem(
                    0)
