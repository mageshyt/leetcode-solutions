...
const rearrangeArray = (nums) => {
  let nums_length = nums.length;
  let positive_nums = []; // ! this helps us to store positive numbers
  let negative_nums = []; // ! this helps us to store negative numbers
  let result = []; // ! here we store our result
  for (let i = 0; i < nums_length; i++) {
    if (nums[i] > 0) {
  ...
  console.log("negative_nums", negative_nums);
  let positive_nums_length = positive_nums.length;
